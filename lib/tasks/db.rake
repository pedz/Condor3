...
namespace :db do
  ...
  task :restore, [:filename] => :load_config do |t, args|
    ...
    sh "pg_restore --clean --no-owner -L /tmp/db.list --single-transaction --dbname=#{config["database"]} #{args.filename}"
  end
  ...
  task :dump, [:filename] => :load_config do |t, args|
    ...
    sh "pg_dump --file=#{args.filename} --format=custom #{config["database"]}"
  end
end
